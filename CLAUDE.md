...
- **同意 UI**: `components/modals/TermsConsentModal.tsx` — `role="alertdialog"` + ModalManager 先頭分岐 (`needsTermsAccept && !isTermsDevBypass()`) で他モーダルより優先 mount。`isTermsDevBypass()` は dev-only `?skip-terms=1` query 評価（PROD ガード + SSR-safe ガード）。M7-α PR-D-2 で導入。Footer 2 link (`legalDocs.ts` の LEGAL_DOCS: 利用規約 / プライバシーポリシー) は Desktop / ProjectSelection / Mobile 全 view に配置。特商法表記 (`tokushou.html`) は2026-07-24、有料プラン未提供のため表記義務対象外と判断し Footer から意図的に除外（ページ自体は残置、有料プラン提供開始時に復活）
- **法務文書 self-host (PR #91)**: `public/legal/{terms-of-service,privacy-policy,tokushou}.{html,md}` + `legal.css` + `legal.js`。HTML は同テンプレで `data-md` 属性違いの 3 ページ。`legal.js` が CDN 経由 marked@13.0.3 + DOMPurify@3.1.7 (CSP scriptSrc の `cdn.jsdelivr.net` で許可済) で md を fetch → parse → `RETURN_DOM_FRAGMENT` で sanitize → `appendChild` で render (innerHTML 不使用)。`legalDocs.ts` の URL は `/legal/*.html` を指す。**正本は `public/legal/*.md`**、`docs/legal/*.md` は履歴/ハンドオフ参照用に残置 (編集時は public/legal を真正本に、必要なら docs/legal にも反映)。LEGAL_REVIEW_REQUIRED 警告（「本文書は実装担当 AI が作成した雛形」等の文言）は **2026-07-24、本田様判断で削除済み**（内容が一般的なものであれば公開可という判断、md 冒頭の HTML コメントおよび本文冒頭の blockquote を除去）。顧問弁護士確認は Tier 2 有料化開始時に再評価する方針は変更なし（詳細 ADR-0001 §開放する課題 2026-07-24 追記）
...
- **Schema**: `BackupV1 { schemaVersion: 1, exportedAt, appVersion, projects, tutorialState, analysisHistory }` (`utils/backupSchema.ts`)。`historyTree` は ADR-0001 の memory-only 方針で export 対象外。M5 (有料化 Tier 2 backup、決済基盤は Stripe / Lemon Squeezy / Paddle 等を着手時に比較選定) / M6 (E2EE) で再利用予定。Legacy bare-project JSON / `{ project: {...} }` envelope は `parseBackup` 内で BackupV1 にラップして後方互換確保。
...
- **個人名を載せない**: `public/dev/` の文面には個人の固有名（氏名・「様」付きの呼称）を書かず、役割名「オーナー」で表記する（2026-09-20 指示、`grep -c '本田' public/dev/index.html` が 0 であること）
