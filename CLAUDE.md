...
- **法務文書 self-host (PR #91)**: `public/legal/{terms-of-service,privacy-policy,tokushou}.{html,md}` + `legal.css` + `legal.js`。HTML は同テンプレで `data-md` 属性違いの 3 ページ。`legal.js` が CDN 経由 marked@13.0.3 + DOMPurify@3.1.7 (CSP scriptSrc の `cdn.jsdelivr.net` で許可済) で md を fetch → parse → `RETURN_DOM_FRAGMENT` で sanitize → `appendChild` で render (innerHTML 不使用)。`legalDocs.ts` の URL は `/legal/*.html` を指す。**正本は `public/legal/*.md`**、`docs/legal/*.md` は履歴/ハンドオフ参照用に残置 (編集時は public/legal を真正本に、必要なら docs/legal にも反映)。LEGAL_REVIEW_REQUIRED 警告は md 冒頭に保持され、ユーザーには未確定 stub であることが見える状態のまま公開される (**2026-06-21 方針**: Tier 0/1 無料運用範囲は「一般的な最低限の自己整備」で stub のまま本番公開済、顧問弁護士確認は Tier 2 有料化開始時に再評価。詳細 ADR-0001 §開放する課題 2026-06-21 更新)
