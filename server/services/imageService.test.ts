...
describe('generateImage - Nano Banana 2 Lite (Gemini 3.1 Flash-Lite Image) 並列2回呼び出し（quota制約による段階生成方式）', () => {
    ...
    it('2件全て reject（RESOURCE_EXHAUSTED 以外、実際の SDK エラー）→ リトライせずラップせず元のエラーをそのまま伝播する（code review 指摘: quota/認証/timeout 分類の維持）', async () => {
        const authError = Object.assign(new Error('UNAUTHENTICATED: invalid credentials'), { code: 16 });
        generateContentMock.mockRejectedValue(authError);
        const { generateImage } = await import('./imageService');

        await expect(generateImage('prompt')).rejects.toBe(authError);
        // リトライしていないことの確認 (2並列 x 1回のみ = 2回)
        expect(generateContentMock).toHaveBeenCalledTimes(2);
    });
    ...
    // fake timers は Promise.allSettled と組み合わせた際にマイクロタスクの処理順序が
    // ズレ、実際にはハンドルされている rejection が一時的に unhandledRejection として
    // 検知される既知の問題があるため使わない (CI の exit code に影響する)。実装側で
    // process.env.VITEST を見て待機時間をミリ秒オーダーに短縮しているため、実時間で
    // 待っても高速に完了する。
    it('2026-07-12: RESOURCE_EXHAUSTED (429) で1回失敗 → 自動リトライして成功する（短間隔の連続生成で prod 実測した現象への対策）', async () => {
    ...
    it('2026-07-12: RESOURCE_EXHAUSTED (429) が最大リトライ回数（3回）を超えて継続 → 最終的に元のエラーをそのまま伝播する（2026-07-12実測: 旧・2回リトライ(合計11秒)でも不足だったため3回(合計70秒)に延長）', async () => {
        const quotaError = Object.assign(new Error('RESOURCE_EXHAUSTED: quota exceeded'), { code: 8 });
        generateContentMock.mockRejectedValue(quotaError);
        const { generateImage } = await import('./imageService');

        await expect(generateImage('prompt')).rejects.toBe(quotaError);
        // 初回 + リトライ3回 = 4回 x 2並列 = 8回
