...
describe('generateImage - Nano Banana 2 Lite (Gemini 3.1 Flash-Lite Image) 並列2回呼び出し（quota制約による段階生成方式）', () => {
    ...
    it('2026-07-12: RESOURCE_EXHAUSTED (429) でも即座に伝播する（リトライしない）。リトライは prod 実測で Vertex AI 側の 1分あたり2リクエスト上限を追加消費し、かえって回復を遅らせる疑いが濃厚だったため撤去（連続生成の防止は呼び出し元 UI 側のクールダウンに委ねる方針）', async () => {
