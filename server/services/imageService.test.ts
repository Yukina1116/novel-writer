...
describe('generateImage - Nano Banana 2 Lite (Gemini 3.1 Flash-Lite Image) 並列4回呼び出し', () => {
    ...
    it('4件全て reject（実際の SDK エラー）→ ラップせず元のエラーをそのまま伝播する（code review 指摘: quota/認証/timeout 分類の維持）', async () => {
    ...
    it('4件中3件が reject し1件成功 → PartialSuccessError のメッセージに元のエラー内容を含める（分類器がsubstringを拾えるように）', async () => {
        generateContentMock
            .mockResolvedValueOnce(fulfilledImageResponse())
