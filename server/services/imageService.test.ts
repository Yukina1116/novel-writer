...
describe('generateImage - Nano Banana 2 Lite (Gemini 3.1 Flash-Lite Image) 並列2回呼び出し（quota制約による段階生成方式）', () => {
    ...
    // 検知される既知の問題があるため使わない (CI の exit code に影響する)。実時間で
    // 待機し、個別テストの timeout を実際のリトライ待機時間 (最大 3000+8000ms) に
    // 合わせて延長する。
