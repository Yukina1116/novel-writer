...
- **3 層プラン**:
  ...
  - Tier 2: 有料 (AI 増枠、Imagen 増枠、E2EE バックアップ opt-in 可)。決済基盤は **Stripe (PSP) または Lemon Squeezy / Paddle / Polar 等 (MoR) を M5 着手時に比較選定**する (2026-06-21 更新、それまで未確定)
    - **2026-09-06 更新**: M5 着手前の予備調査として、決済基盤の一次情報比較（Stripe Managed Payments の日本事業者対応・Firebase 決済連携拡張機能の 2027-03-31 提供終了確定 等）を実施。詳細は `.claude/memory/payment_provider_reference_2026-09.md` 参照。着手時期は本田様判断待ちで未定（現時点: 準備は継続するが Stripe Managed Payments の利用審査申請はまだ行わない）
    - **2026-09-06 追記**: サービス規模がまだ小さい段階のため、本田様判断により決済基盤は **まず審査不要な標準 Stripe（Billing + Tax）でシンプルに始める** 方針に確定。Managed Payments（消費税・請求書処理までまとめて代行する追加オプション、事業形態・地域による利用資格審査あり）は規模拡大後に改めて検討する後回し事項とし、M5 の当面のスコープからは外す
...
- 課金導入時の法務作業（利用規約、特商法、プライバシーポリシー）
  ...
  - **2026-09-06 更新**: 特商法表記の追加タイミングについて、手続き最小化の方針で合意。決済基盤（価格・支払方法・引渡時期・返品ポリシー等）が未確定な間の書き直しを避けるため、それらが確定した後の Tier 2 限定公開開始時点で初めて必要事項を記載する。**ただし特商法表記は「一度書けば終わり」の一括対応ではなく、掲載後に価格・支払方法・引渡時期・返品ポリシー等の実際の商取引条件が変わるたびに、その都度追随して更新する法的義務が継続する**（消費者向け表示の正確性維持のため）。顧問弁護士確認は 2026-07-24 方針（一般的な内容であれば追加確認不要）を踏襲し、初回掲載・その後の条件変更時の更新のいずれについても新たな必須手続きとしては課さない
