...
- 課金導入時の法務作業（利用規約、特商法、プライバシーポリシー）
  ...
  - **2026-09-06 更新**: 特商法表記の追加タイミングについて、手続き最小化の方針で合意。決済基盤（価格・支払方法・引渡時期・返品ポリシー等）が確定した後に、Tier 2 の限定公開開始時点で一度だけ必要事項を機械的に追記する（価格改定のたびの書き直しを避ける）。顧問弁護士確認は 2026-07-24 方針（一般的な内容であれば追加確認不要）を踏襲し、Tier 2 開始時点でも新たな必須手続きとしては課さない
