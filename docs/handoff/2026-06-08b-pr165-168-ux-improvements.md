...
- Owner: yasushi-honda
- Status: ✅ **PR #165–#168 4 件すべてマージ + Cloud Run デプロイ完走**。PR-A/B/C は本番 Playwright MCP 実機確認まで完走。PR-D1 (タイトル自動同期) のみ実機リンク済みデータでの最終確認を本田様にお預け
...
| **#168** | feat(sync): タイムライン⇔プロットボードのタイトル差異を保存時に自動同期 + トースト通知 (2 files, +330/-19) | ✅ `bc5da5a` + 本田様実機確認待ち |
...
- **実機確認 (本田様お任せ)**: 本セッション内の Playwright MCP では「タイムラインへ送る」アイコン経由でリンクを作る経路で、PlotBoardModal の localState と store 間の状態同期問題と思われる挙動でタイムラインモーダルに対応イベントが表示されない状態に到達。これは PR-D1 由来ではなく既存の `createEventFromPlot` + `handleSavePlotBoard` の保存タイミング起因と推測 (本セッションスコープ外)
...
|---|------|-------|------------------|--------------|
| 1 | **PR-D1 (#168) のリンク済みデータ実機確認** | A（指示待ち） | 本田様の既存リンク済みアイテムのプロジェクトでタイトル変更 → 同期 + トースト確認 | 本田様による目視確認、不具合発見時は Issue 起票検討 |
| 2 | **「タイムラインへ送る」後のタイムライン未反映**の挙動切り分け | B（検出は read-only 可、修正は人指示待ち） | 本田様が「これは本当に既存バグか調査して」と指示 | `createEventFromPlot` / `handleSavePlotBoard` / `PlotBoardModal.tsx` の状態同期を調査、再現確認、Issue 起票判断 |
| 3 | `novel-writer-prod` への構築着手 | C（起点指示済み） | 本田様が「bugfix 一通り完了」と判断したタイミング | impl-plan → GCP リソース構築 → workflow 分岐 → Firebase Secrets 登録 → smoke test |
| 4 | `./scripts/setup-safety-event-metrics.sh --project novel-writer-dev` 実行 | A（指示待ち） | 本田様からの実行指示 | metrics 初期化 |
| 5 | Cloud Logging baseline 観察 → alert enable 判断 | A（指示待ち） | 本田様からの判断結果（1〜4 週間後） | alert 設定変更 |
| 6 | Issue #137 残サブ #6 / #8 / Issue #147 / #152 / #155 / #156 の優先順位決定・実装着手 | B 修正 / C（起点指示済み） | 本田様からの番号単位明示指示 | impl-plan → 実装 |
...
| 7 | 「タイムラインへ送る」未反映の自主修正 | C（起点 unclear） | 切り分け調査前の修正提案は越権 |
...
| 残留プロセス | ✅ なし |
| 即着手タスク | 0 件 |
| 条件待ち | 6 件 (全て decision-maker trigger) |
...
- PR #165–#168 4 件すべて完走（マージ + デプロイ）
- PR-A/B/C は本番 Playwright MCP 実機確認まで完走
- PR-D1 はコードロジック品質保証完了 (unit test 9 件 + Codex + code-review + CI)、実機リンク済みデータの確認のみ本田様にお預け
- 条件待ち 6 件すべて decision-maker からの trigger 待ち
