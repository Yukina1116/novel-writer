...
- Phase 2 で deploy された Cloud Run service `novel-writer @ novel-writer-prod` は本田様の dev test 用として継続稼働中
- Phase 3 で dev → prod 運用フロー (deploy 判断 / tag / rollback / データ同期) を以下に文書化:
  - 規範 (なぜ): [ADR-0002](../adr/0002-dev-prod-deploy-flow.md)
  - 手順 (どう): [runbook prod-deploy-flow.md](./prod-deploy-flow.md)
  - タスク + AC: [phase3-tasks.md](../spec/prod-migration/phase3-tasks.md)
- Phase 3 で本 runbook の Rollback 手順 (3+1 段階) は Phase 4 着手後の追補対象:
  - 段階 1〜3 (公開即遮断 / revision 切替 / service delete) は [runbook prod-deploy-flow.md §rollback 判断と 3 段階手順](./prod-deploy-flow.md#rollback-判断と-3-段階手順) に縮約・正式化
  - 段階 4 (Firestore データ rollback) は Phase 4 で PITR と一体再設計 (Phase 3 では Phase 3 segment スコープ外)
- Phase 4 (一般公開) 着手前の GO チェックは [phase3-tasks.md §Phase 4 GO チェック](../spec/prod-migration/phase3-tasks.md#phase-4-go-チェック) を参照
  - 法務確認 / 課金クォータ / Firestore PITR / Cloud Logging dashboard / SLO・incident policy / 本田様明示 GO
