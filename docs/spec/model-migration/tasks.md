...
### I. dev/prod 実機再検証（段階呼び出し方式、H 完了後）— dev 完了、prod 未実施
...
- [ ] ~~「追加生成」ボタンが成功し、既存2枚を残したまま新規2枚が追記されることを確認~~ → **未達成のまま保留**。dev実機で計6回試行したが、初回成功（08:15頃）以降15分以上にわたり quota 超過（429）が継続し、無関係な穏当なプロンプトに変更しても再現。追記マージロジック自体は `ImageGenerationModal.handlers.test.ts` の static pin + 4種のコードレビュー（code-review/evaluator/review-pr/codex）で論理的に検証済みのため、これ以上のdev実機での再試行は行わず prod デプロイに進む（本田様承認済み、2026-07-05）
- [ ] Cloud Run ログでエラー無し・想定通りの `PartialSuccessError`/成功ログを確認 → prod 検証時に実施
