...
- **Status**: PR #230 実装 + PR #231 リージョン修正は **完了・実機検証済み**。本PR（画像生成の段階呼び出し化・quotaバグ修正）は **実装・単体テスト完了、prod 実機再検証は未実施**（タスク I、マージ・デプロイ後に実施予定）
...
### I. prod 実機再検証（段階呼び出し方式、H 完了後）— 未実施
- [ ] feature ブランチ → PR 作成 → 番号認可マージ → dev/prod デプロイ
- [ ] Playwright MCP で「AI 立ち絵生成」を実行し、初回2枚の data URI が返ることを確認
- [ ] 「追加生成」ボタンを押し、既存2枚を残したまま新規2枚が追記されることを確認（quota window 内で 429 になった場合は、既存の quota エラー文言が表示されエラーバウンダリが壊れないことを確認）
- [ ] Cloud Run ログでエラー無し・想定通りの `PartialSuccessError`/成功ログを確認
...
3. 画像生成 API（人物プロンプト）→ 200 + **2 枚**の data URI 配列、または一部失敗時は成功比率に応じた `PartialSuccessError`（検証: 実機・Playwright MCP、タスク I）— ⏳ **未実施**（コード実装・テスト・lint は完了、本 PR マージ・デプロイ後に実機検証予定）
...
9. **（本PR で追加）** 画像生成1回あたりの並列呼び出し数が Vertex AI quota（2 req/分）以下に収まる設計であること（quota超過を「常に起こる」構造的バグから「同一1分ウィンドウ内の連続操作でのみ起こり得る」例外的事象に縮小する）。初回生成2枚 + 追加生成ボタンで2枚ずつ追記できること（検証: 実機・Playwright MCP）— ⏳ **未実施**（設計・実装・単体テストは完了。タスク I で実機検証予定。「追加生成」連打時の429は既知のスコープ外事象、リスク欄参照）
...
- **「追加生成」ボタンのクールダウン非実装（意図的なスコープ外）**: quota=2 req/分のため、初回生成の直後（同一1分ウィンドウ内）に「追加生成」を押すと 429 になり得る。本田様の「すぐ簡単に追加生成させれる」という意図を優先し、クールダウン待機UIは実装しない設計判断（Codex `plan` mode セカンドオピニオンでも同リスクを指摘済み）。429 時は既存の quota エラー文言がそのまま表示され、既存の生成済み画像は失われない。将来的に UX 改善（残り時間表示等）が必要になれば [novel-writer#232](https://github.com/Yukina1116/novel-writer/issues/232) のスコープで検討する。cross-project 注記: このquotaはGCPプロジェクト単位（全ユーザー共有）のため、複数ユーザーが同時に画像生成を行うと、個々のユーザーの月間利用枠には余裕があっても quota 超過が起こり得る（本PRのスコープでは対応せず、実運用で顕在化した場合に再評価）
