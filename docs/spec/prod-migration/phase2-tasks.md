...
| AC-P2-7 | Vertex AI smoke test `/api/ai/utility/names` 200 + 名前候補返却 | 本田様 login + Playwright |
| AC-P2-8 | rollback 手順 (`update-traffic --to-revisions`) が runbook に明文化 | grep |
| AC-P2-9 | 本ドキュメントの全チェックボックス `[x]` | grep |
...
- ADR-0001 §M0 (max-instances=2 + 予算アラート、Phase 2 deploy にも適用)
- ADR-0001 §M3 (Cloud Run public + verifyIdToken、AC-13 で検証)
