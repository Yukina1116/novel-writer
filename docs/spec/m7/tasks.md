...
- Status: 🟡 コードベース完了 / 法務承認 + manual E2E は人間作業として保留 (2026-04-29 時点)
  - AI 範囲のすべての実装タスクは [x] (PR-A〜D-2 全 merge)
  - L231 (E2E 手動確認) と L240 (法務承認取得) は **(HUMAN-WORK)** で `[ ]` のまま — Claude は実行不可、ユーザー側完了で `[x]` に切替
- Owner: yasushi-honda
- Started: 2026-04-28
...
- [ ] **(HUMAN-WORK)** dev サーバーで E2E フロー手動確認:
...
- [ ] **(HUMAN-WORK) 本番公開前の MUST**: 法務 stub 3 文書をユーザー / 顧問弁護士に送付し承認取得（AI セッション外で実施）
