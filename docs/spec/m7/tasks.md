...
| PR-D | feat(m7) 同意 UI + Firestore スキーマ拡張 + footer link | 大 | 1.5〜2 時間 | ⏳ PR-A merge 後 |

着手順序: **PR-A / B / C を 3 並列着手 → PR-A merge 後に PR-D 着手**
...
- [ ] AC-1 / AC-2 (本ドキュメント `acceptance-criteria.md` 参照): 法務 stub 3 文書 + spec ドキュメント存在 + TODO マーカー検出可能
- [ ] markdown link 切れなし (relative path 確認)
- [ ] ADR Tier 1 仕様の修正が `docs/spec/m3/usage-cost-config.md` と一貫
...
- [ ] `server/middleware/errorHandler.ts`: `logger.error({ severity: 'ERROR', requestId, route, code, ... })`
- [ ] `server/middleware/withUsageQuota.ts`: reserve/commit/cancel イベントを `logger.info`
- [ ] `server/middleware/verifyIdToken.ts`: transient 系を `logger.warn`、permanent を `logger.info`（auth 失敗は通常運用）
- [ ] `server/index.ts`: 起動 / probe 失敗を logger 経由
- [ ] `server/routes/users.ts` 等の残存 console.error を全て置換
- [ ] 検証: `grep -r "console.error" server/` で 0 件

### Acceptance Criteria (PR-B)

- [ ] AC-3 (acceptance-criteria.md 参照)
- [ ] 既存テスト全 PASS（errorHandler.test, withUsageQuota.test 等が logger mock で動作継続）
- [ ] `npm run dev` で実際にリクエスト発火 → JSON 形式で stdout 出力されることを目視確認
...
- [ ] `components/AppErrorBoundary.tsx` 新規 (class component, React 18 patterns):
...
- [ ] `hooks/useGlobalErrorHandlers.ts` 新規:
  ...
  - cleanup で remove
- [ ] `hooks/useGlobalErrorHandlers.test.ts`:
  - mount/unmount で listener 登録/解放
  - event 発火で showToast 呼出

#### C.3 統合
- [ ] `index.tsx` で `<AppErrorBoundary>` で `<App>` を wrap
- [ ] `App.tsx` で `useGlobalErrorHandlers()` 呼出（auth 完了後等の早い段階）

### Acceptance Criteria (PR-C)

- [ ] AC-4 (acceptance-criteria.md 参照)
- [ ] vitest 全 PASS
...
- [ ] `types.ts` の `User` (or 該当型) に `termsAcceptedAt: Timestamp | null` / `termsVersion: string | null` 追加
- [ ] `firestore.rules`: users/{uid} の update 許可フィールドに `termsAcceptedAt`, `termsVersion` 追加（rules unit test 更新）
- [ ] 定数 `TERMS_VERSION = '2026-04-28'` を `server/services/constants.ts` 等に export

#### D.2 BE accept-terms route
- [ ] `server/routes/users.ts` に `POST /api/users/accept-terms` 追加:
  - `verifyIdToken` 経由
  - body: `{ termsVersion: string }`
  - Firestore `users/{uid}` に `{ termsAcceptedAt: serverTimestamp(), termsVersion }` を transaction update
  - レスポンス: 200 + 更新後の値
- [ ] `server/routes/users.test.ts` に accept-terms ケース追加
- [ ] `/api/users/init` のレスポンスに `termsAcceptedAt` / `termsVersion` を含める（FE が初回判定に利用）

#### D.3 authSlice 拡張
- [ ] `store/authSlice.ts` に `needsTermsAccept: boolean` / `termsAccepting: boolean` 追加
- [ ] users/init レスポンスに基づき `needsTermsAccept = (termsAcceptedAt === null) || (termsVersion !== TERMS_VERSION)` を設定
- [ ] `acceptTerms()` action 追加（fetch /api/users/accept-terms → 成功で needsTermsAccept=false）

#### D.4 TermsConsentModal
- [ ] `components/modals/TermsConsentModal.tsx` 新規:
  - 同意前は他モーダル / 主要操作をブロック（z-index 最上位、close ボタンなし）
  - 3 文書 link (新タブ)、「同意して開始」ボタン
  - クリックで `acceptTerms()` 呼出、loading / error handling
- [ ] `components/ModalManager.tsx` に組み込み

#### D.5 Footer link
- [ ] `components/Footer.tsx` 新規 (or 既存改修):
  - 利用規約 / プライバシーポリシー / 特商法 への link (`target="_blank" rel="noopener"`)
  - link 先は `/legal/terms-of-service.html` 等の static path（実体は dist/ にコピー or md viewer ルートを別途検討）
  - **暫定**: M7-α では GitHub repo 上の md ファイルへの link で開始（本番公開時に static page に置換）
- [ ] App.tsx / App.mobile.tsx に Footer を配置

#### D.6 dev bypass
- [ ] `?skip-terms=1` query parameter で同意モーダルを dev 環境で skip 可能に（`NODE_ENV !== 'production'` ガード）

### Acceptance Criteria (PR-D)

- [ ] AC-5, AC-6, AC-7 (acceptance-criteria.md 参照)
- [ ] `npm run test` PASS、`npm run lint` 0 errors、`npm run build` 成功
- [ ] /simplify → /safe-refactor → Evaluator 分離プロトコル実施（5 ファイル以上 + 新機能のため必須）
- [ ] /review-pr 6 並列 + /codex review セカンドオピニオン
...
- [ ] CI (Cloud Run deploy) 通過
- [ ] dev サーバーで E2E フロー手動確認:
  ...
  6. `Promise.reject('test')` → toast 表示
  7. BE エラー発火 → `npm run dev` で JSON 形式 log 確認
- [ ] handoff 更新 (`docs/handoff/LATEST.md`)
- [ ] **本番公開前の MUST**: 法務 stub 3 文書をユーザー / 顧問弁護士に送付し承認取得（AI セッション外で実施）
