...
- 認証ゲート実装本体（M3 で実装）
- IndexedDB 移行（M2 で実装）
- 課金連携 (M5 で実装、決済基盤は Stripe / Lemon Squeezy / Paddle 等を着手時に比較選定、ADR-0001 2026-06-21 更新参照)
- E2EE バックアップ（M6 で実装）
...
### タスク
- [x] Firebase Console で `novel-writer-dev` プロジェクトの Authentication を有効化
- [x] Firebase Console で Sign-in method の Google プロバイダを有効化
- [x] `package.json` に追加: `firebase` (FE), `firebase-admin` (BE), `firebase-tools` (devDep)
- [x] `firebase.json` 新規作成（emulators: auth のみ、port 9099）
- [x] `.firebaserc` 新規作成（projectId: novel-writer-dev）
- [x] `.gitignore` に Firebase debug log 追加: `firebase-debug.log`, `firestore-debug.log`, `ui-debug.log`, `*.log`
- [x] `server/firebaseAdmin.ts` 新規作成（admin SDK 初期化スタブ、まだルートでは使わない）
- [x] `package.json` の scripts に追加:
  - [x] `dev:emu`: Firebase Auth Emulator + Vite を並列起動（concurrently 利用）
  - [x] `test:firebase-admin`: verifyIdToken の単発検証スクリプト実行
- [x] `scripts/test-firebase-admin.ts` 新規作成（Emulator 経由で取得した idToken を verify）

### Acceptance Criteria
- [x] **C1**: Firebase Console で Authentication > Sign-in method > Google が "有効" 表示（2026-04-26 ユーザー確認、スクショを PR コメントに添付）
- [x] **C2**: `npm run dev:emu` で Firebase Auth Emulator が `localhost:9099` で起動し、`curl -s http://localhost:9099` が 200
- [x] **C3**: `npm run test:firebase-admin` でテストスクリプトが PASS（Emulator から idToken 取得 → admin.verifyIdToken() で uid 取得）
