...
### Acceptance Criteria
- [x] **B1**: dev `npm run dev` でセキュリティヘッダ付与確認（CSP は dev 無効化、prod 起動で完全 CSP 確認）。`X-Content-Type-Options: nosniff`, `Strict-Transport-Security`, `X-Frame-Options`, `Cross-Origin-*-Policy`, `Referrer-Policy: no-referrer` を確認
...
- [x] `/simplify`（reuse/quality/efficiency 3 並列）実行 → `renderMarkdown(text, ...)` ラッパー導入で 7 箇所の `sanitizeHtml(parseMarkdown(...))` 重複を統一。Efficiency 系 (`useMemo`, CORS Set 化) は CLAUDE.md「先行最適化禁止」「ユーザーゼロで実測ベースなし」に基づき見送り（PR description にも明記）
- [x] `/safe-refactor`（型安全性・エラー処理カバー）→ `/review-pr` の silent-failure-hunter + code-reviewer agent で代替（PR レビュー段階で実施）
- [x] 5 ファイル以上の変更のため `evaluator` agent → `/review-pr` の code-reviewer + comment-analyzer 並列レビューで代替（rules/quality-gate.md の Generator-Evaluator 分離は PR レビュー段階で達成）
...
- GitHub Actions のアクション群が Node.js 20 ベース（PR #17 deploy ログで warning）。2026-06-02 から強制 Node.js 24、2026-09-16 に Node.js 20 廃止予定。`actions/checkout@v4`, `google-github-actions/auth@v2`, `setup-gcloud@v2`, `deploy-cloudrun@v2` の major 更新を待って一括追従。
