...
| PR-F | usage クォータ実装 (transaction 予約 + requestId 冪等 + コスト上限) + firestore.rules `usage` コレクション追加 + 持越/PR-E 反映 + Issue #40 | 大 | 4〜6 時間 | ✅ 完了 (PR #45) |
| PR-G | FE 統合 (apiCall Bearer + requestId 自動生成 + 共通分類器 + 持越 #2) + Cloud Run `--allow-unauthenticated` + Firestore rules 本番デプロイ | 中 | 2〜3 時間 | ✅ 完了 (PR #46) |
...
ブランチ: `feature/m3-pr-g-fe-integration`（merge 済 PR #46）

### 背景

PR-D/E/F で確立した BE 認証 (`verifyIdToken`) + usage クォータ (`withUsageQuota`) を FE
側から正しく利用できる状態にする。`apiCall` に Bearer 付与 + requestId 自動生成 +
401/429/503/409 共通分類器を導入し、Cloud Run を `--allow-unauthenticated` で public 化
することで M3 マイルストーンを完成させる。

### タスク

#### G.1 apiClient.ts 改修
- [x] `getIdToken()` で Authorization: Bearer 自動付与
- [x] `crypto.randomUUID()` で `requestId` 自動生成、呼出元指定があれば上書きしない
- [x] `AuthGateErrorCode` 列挙拡張: `AUTH_REQUIRED|AUTH_INITIALIZING|AUTH_EXPIRED|QUOTA_EXCEEDED|SERVICE_UNAVAILABLE|DUPLICATE_REQUEST|NETWORK_ERROR|REQUEST_ABORTED|SERVER_ERROR`
- [x] `ApiCallError.code` を required 化（コメントと型の整合、silent fallthrough 排除）
- [x] `classifyHttpError`: 401→AUTH_EXPIRED / 429+QUOTA_EXCEEDED→QUOTA_EXCEEDED / 503/504→SERVICE_UNAVAILABLE / 409+DUPLICATE_REQUEST→DUPLICATE_REQUEST / その他 4xx/5xx→SERVER_ERROR / fetch 例外→NETWORK_ERROR / AbortError→REQUEST_ABORTED
- [x] `getIdToken` 失敗時に `auth/network-request-failed` を SERVICE_UNAVAILABLE に分離（refresh token revoke と一時的 network 障害を区別）

#### G.2 authSlice.ts: needsUserInit retry signal (M2 持越 #2)
- [x] `needsUserInit: boolean` flag 追加
- [x] `retryUserInit()` メソッド追加、AI 呼出前に 1 度 retry
- [x] `inFlightUserInitRetry` module-scope guard で並列多重発火防止
- [x] retry throw 経路で `needsUserInit=false` リセット（永続失敗ループ防止）
- [x] `UserInitError` 命名 + `isUserInitError` type guard で foreign error の transient 誤分類排除
- [x] `isTransientUserInitError`: 503/504/0 (network 断) を transient 判定
- [x] `__testing.resetInFlightUserInitRetry()` export（test 間 module-scope state リーク防止）

#### G.3 Cloud Run public 化
- [x] `.github/workflows/deploy.yml` に `--allow-unauthenticated` + `NODE_ENV=production` 追加
- [x] invariant コメント追加（verifyIdToken middleware 保護前提を明示）
- [x] merge 後 main deploy → curl で 401 確認（DoD 達成 2026-04-27）

#### G.4 Firestore rules 本番デプロイ
- [x] `firebase deploy --only firestore:rules -P novel-writer-dev` 実行（usage コレクション全拒否を本番反映）

#### G.5 テスト
- [x] `apiClient.test.ts` 新規 (33 ケース): ensureRequestId / classifyHttpError / apiCall (Bearer / requestId 自動生成 / 4 段ガード / 共通分類 / retry / network 断 / SERVER_ERROR / 504 / AbortError / cross-layer in-flight)
- [x] `store/authSlice.test.ts` 新規 (5 ケース): retry 成功 / throw 経路で flag リセット / in-flight 共有 / 未認証 throw / network 断 status=0

#### G.6 ドキュメント整備
- [x] ADR-0001 ロードマップ表 M3 ✅ + 振り返りセクション追記
- [x] CLAUDE.md "AI API 層" 表に withUsageQuota / sen 単位を反映、authSlice に needsUserInit/retryUserInit を反映
- [x] 本ファイル PR-G セクション ✅ 詳細化
- [x] `docs/handoff/LATEST.md` を M3 完了状態に更新
...
- [x] **G3**: 401 → AUTH_EXPIRED「ログイン期限が切れました」、429 → QUOTA_EXCEEDED「今月の AI 利用枠の上限」、503/504 → SERVICE_UNAVAILABLE、409 → DUPLICATE_REQUEST、500/その他 → SERVER_ERROR、fetch 例外 → NETWORK_ERROR、AbortError → REQUEST_ABORTED
...
- [x] **G7**: PR merge 後、`curl -i $URL/api/ai/utility/names -H 'Content-Type: application/json' -d '{"requestId":"manual-check-..."}'` → **HTTP/2 401 確認済**（DoD 達成 2026-04-27）
- [x] **G8**: 本番 Firestore rules デプロイ（usage コレクション全拒否反映）
- [x] **G9**: ADR-0001 ロードマップ表 M3 ✅、M3 振り返りセクション追記
- [x] **G10**: lint 0 / vitest 176 / firestore-rules 20 / build PASS

### 品質ゲート

- [x] `npm run lint` PASS
- [x] `npm run test` PASS (176/176)
- [x] `npm run build` PASS
- [x] `npm run test:firestore-rules` PASS (20/20)
- [x] `/simplify` (3 並列) → 4 件反映
- [x] `/review-pr` (5 エージェント並列) → 8 件反映 (HIGH/Critical 全対応)

### M4 以降への持越事項

- **残量バー UI**: usage rules 緩和 + コンポーネント追加。M4 Export/Import UI と一緒に or M5 Stripe 連携時に枠拡張動線とセットで実装検討
- **Stripe 連携 (M5)**: Tier 2 (paid) の `users.plan` 動的取得、users/init で plan 反映、`withUsageQuota` で Tier 別上限切替
- **observability 拡張**: Vertex AI 応答 `usage_metadata` から token 数を取得して `commit` の actualCost を補正（PR-F は固定 estimatedCost で運用）
- **401 自動 sign-out**: PR-G では文言誘導のみ。N 回連続 401 で自動 signOut + 再ログインダイアログを別 PR で実装
- **コードベース review 持越 (rating 5-7)**:
  - Sen branded type で UsageDoc の単位安全性
  - AiRouteKey ↔ Express path 単一レジストリ化（drift 防止）
  - ReservationHandle の Date 露出 → 内部 docId 化
  - AuthedRequest assertion 関数化
  - processedIds sliding window drop の警告ログ
  - AC F8 動的検証（route ファイル全件 withUsageQuota ラップ確認）

---

## M3 完了の定義

- [x] PR-D AC 全 PASS、merge 済 (PR #37)
- [x] PR-E AC 全 PASS、merge 済 (PR #39)
- [x] PR-F AC 全 PASS、merge 済 (PR #45) ※ Issue #40 同梱解消
- [x] PR-G AC 全 PASS、merge 済 (PR #46)
- [x] ADR-0001 ロードマップ表 M3 ✅ 完了に更新
- [x] `docs/handoff/LATEST.md` 更新（次マイルストーン M4 への申し送り）
- [x] Cloud Run public 化後の 401 curl 確認 (DoD)
- [x] 本番 Firestore rules デプロイ (DoD)
