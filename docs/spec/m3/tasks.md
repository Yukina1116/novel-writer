...
- [x] **D7**: `sanitizeForUpdate<T>(...)` の戻り値型が undefined フリーで型表現される（型レベルテスト or 直接 hover で確認）
...
### PR-E 持越（PR-D evaluator 評価で発覚した残課題）
...
- **`auth/quota-exceeded` の transient 分類**: 現状 permanent (401) に落ちる。Firebase Admin SDK が quota 超過時にこの code を投げる場合があり、ユーザー再認証ループに入るリスクあり。PR-E で公式仕様確認後 `TRANSIENT_AUTH_CODES` 追加を検討
