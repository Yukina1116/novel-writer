...
> **2026-07 追記**: テキストモデルは `gemini-3.1-flash-lite`（入力 $0.25/1M・出力 $1.50/1M、
> 旧モデルより安価）、画像モデルは `gemini-3.1-flash-lite-image`（Nano Banana 2 Lite、
> 1 回 = 4 並列呼出）へ移行済み。上記の sen 見積もりは旧モデル基準のままで、実コストへの
> 追従は未実施（docs/spec/model-migration/tasks.md タスク G で Billing 実測後に再評価予定）。
> 新モデルは両方向とも旧モデルより単価が低いため、現行の `MONTHLY_LIMIT_SEN` / `ROUTE_COST_SEN`
> は安全側（より保守的なマージン）に倒れている。
...
| image/generate | 1000 | 10 | Nano Banana 2 Lite 4 並列リクエスト = 4 画像生成、最も高コスト |
