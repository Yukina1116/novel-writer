...
- **FR-4**: `server/utils/promptSafety.ts` の `safetyEvent` 参照 6 箇所 (createWarnAggregator 呼出 4 callsite + `createDepthExceededAggregator` helper 内 1 件 + `logger.warn` での `HISTOGRAM_OVERFLOW` 直接参照 1 件) を文字列リテラルから `SAFETY_EVENTS.*` 参照に切り替える (logic 不変、`grep -c "SAFETY_EVENTS\." server/utils/promptSafety.ts` = 6 で検証)
...
- alert policy は scaffold (stdout 出力のみ) で gcloud 呼出なし。実 create は runbook §5.2 で本田様が Console 経由で実施 (notification channel ID 環境依存のため script から hardcoded 不可)
...
AC-3: server/utils/promptSafety.ts の safetyEvent 参照箇所 (createWarnAggregator
      4 callsite + createDepthExceededAggregator helper 内 1 件 +
      logger.warn での HISTOGRAM_OVERFLOW 直接参照 1 件 = 計 6 箇所) が文字列
      リテラルではなく SAFETY_EVENTS.* 経由になっている
      (`grep -c "SAFETY_EVENTS\." server/utils/promptSafety.ts` = 6 で検証)
...
AC-7: 既存 promptSafety.test.ts 全件 + 新規 test が PASS。
      Tests: 全件 PASS (この PR で +16 件追加: 7 unit (T1) / 4 lockstep (T3) / 5 bash syntax (T4))
      file 内訳: 38 → 39 (+1: promptSafetyEvents.test.ts)、T3/T4 は既存 tests/static/ に追加
...
- **OQ-1**: alert notification channel として使う email アドレス / Slack webhook
  - 候補: `hy.unimail.11@gmail.com` 等、本田様運用想定によって決定
