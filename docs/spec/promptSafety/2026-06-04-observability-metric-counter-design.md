...
AC-7: 既存 promptSafety.test.ts 全件 + 新規 test が PASS。
      Files: 38 → 39 (+1: promptSafetyEvents.test.ts) ※ T5/T6 は既存 tests/static/
      に追加、新規 file は T1 のみ。
      Tests: 619 → 635 (+16: T1 = 7 tests、T5 = 4 tests、T6 = 5 tests)
      (※ M-1 修正で T6 が 3 → 5 tests に増加、AC-9 検証含む完成形)
